jQuery(function($){
    ...
    window.speedR = 75;
    window.bulkR = 20;
    window.showPrize = function(prize, finalize = false, resetTimer = false) {
        if (resetTimer) window.timerR = 0;
        window.timerR++;
        setTimeout(function(prize){
            $('.sc1__tree .gift .item').removeClass('active');
            $('.sc1__tree .gift:nth-child('+prize+') .item').addClass('active');
            setTimeout(function () {
                if (finalize) {
                    alert(prize);
                }
            }, 1000, finalize);
        }, window.speedR * window.timerR, prize, finalize);
    };

    $('body').on('click', '.sc1__cta-tree-block', function (e) {
        e.preventDefault();
        $.ajax({
            type: "POST",
            data: {
            ...
            success: function (data) {
                ...
                if (data.success) {
                    ...
                    let bulkIters = window.bulkR * 6;
                    let tries = data.prize + bulkIters;
                    for (let i = 1; i <= tries; i++) {
                        showPrize(i%6 || 6, i == tries, i == 1);
                    }
                } else {
