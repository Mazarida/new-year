        ...
        switch ($_POST['actiond']) {
            case 'round-wheel':
                ...
                if ($tries > 0) {
                    ...
                    if (!add_promocode($prix)) {
                        add_tries(1);
                        add_tx_log(1, "Возвращена попытка за неимением промокодов, приз #".$prix);
                        $res->success = false;
                        $res->error_mess = "Закончились промокоды для выпавшего приза, попытка возвращена";
                    }
